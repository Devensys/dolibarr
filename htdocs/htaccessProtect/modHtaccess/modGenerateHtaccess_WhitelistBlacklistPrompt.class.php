...
class modGenerateHtaccess_WhitelistBlacklistPrompt extends modGenerateHtaccess
{
    function __construct($bddips, $accountList, $langs)
    {
        ...
        $this->desc = $this->langs->trans("WhitelistBlacklistPromptDesc");
    }
    ...
    function Info(){
        ...
        if(count($this->ipwhite) && count($this->ipblack) && count($this->accountList)){
        ...
        else if(!(count($this->accountList) || count($this->ipwhite))){
            $return[0] = 3;
            $return[1] = $this->langs->trans("ConfigurationUserOrWhiteNeeded");
            return $return;
        }

        else if(!count($this->ipwhite) && !count($this->ipblack)){
            $return[0] = 2;
            $return[1] = $this->langs->trans("ConfigurationNoWhiteBlack");
            return $return;
        }
            ...
            $return[0] = 2;
            $return[1] = $this->langs->trans("ConfigurationNoBlack");
            return $return;
        }
